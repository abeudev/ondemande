...
class WebSiteAppSettingsScreen extends StatefulWidget {
  @override
  _WebSiteAppSettingsScreenState createState() => _WebSiteAppSettingsScreenState();
}

class _WebSiteAppSettingsScreenState extends State<WebSiteAppSettingsScreen> {
  ...
  _getList(){
    ...
    list.add(Row(
      children: [
        Expanded(child: SelectableText(strings.get(418),                       /// "WebSite App Settings",
    ...
    Widget _mainColor = Row(
      children: [
        ...
        Container(
          width: 150,
          child: ElementSelectColor(getColor: (){return appSettings.websiteMainColor;},
            setColor: (Color value){
              appSettings.websiteMainColor = value;
              _redraw();
            },),
        )
      ],
    ...
    if (bytesImage != null) {
      ...
      if (appSettings.websiteLogoServer.isNotEmpty) {
        _image = Image.network(appSettings.websiteLogoServer);
        _needDelete = true;
      }
    }
          ...
          button2b(strings.get(420), () async {  /// "Delete image"
            if (bytesImage != null){
              ...
              if (appSettings.providerLogoServer.isNotEmpty){
                await deleteImage("webSiteAppLogo");
                _redraw();
              }
            }
          }),
    ...
    list.add(Center(child: button2b(strings.get(421), () async { /// Restore settings
      appSettings.websiteMainColor = Color(0xff69c4ff);
      bytesImage = null;
      if (appSettings.websiteLogoServer.isNotEmpty)
        await deleteImage("webSiteAppLogo");
    ...
    list.add(Center(child: button2b(strings.get(9), () async {  /// "Save"
      if (bytesImage != null){
        var ret = await uploadImage("webSiteAppLogo", bytesImage!);
        if (ret != null)
